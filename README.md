...
**Permission-aware Agentic RAG Backend** — 개인, 그룹, system knowledge를 권한 경계 안에서 검색하고, 출처와 실행 과정을 함께 남기는 AI 채팅 제품의 백엔드입니다.
...
| 질문 | 이 프로젝트의 답 |
| --- | --- |
| 무엇을 만들었나? | 개인/그룹/system knowledge를 이용해 출처가 있는 답변을 생성하는 FastAPI + LangGraph 백엔드 |
...
- **Hybrid retrieval**: pgvector 후보와 request-local `BM25Okapi` 후보를 독립적으로 수집하고 `chunk_id` 기준 RRF(`k=60`)로 결합한 뒤 reranking/context packing을 수행합니다.
- **Inspectable agent flow**: `general_assistant` LangGraph가 source-selection gate, RAG Agent, opt-in memory, response node를 명시적인 state로 연결합니다.
- **Product-owned state**: conversation, run, message, citation, redacted event는 Product DB가 소유합니다. LangGraph state를 사용자 transcript의 source of truth로 사용하지 않습니다.
- **Streaming product contract**: SSE로 진행 이벤트, agent trace, answer delta, 완료/실패 상태를 전달하고 같은 결과를 서버에 저장합니다.
- **Offline-first verification**: deterministic provider/embedding/reranker 경계를 유지해 테스트가 API key 없이 실행됩니다.

## 측정된 성능 개선

아래 수치는 공개 SLA가 아니라 동일 시나리오를 비교한 **local profile**입니다. 최적화 전후에 retrieval shape 또는 parser/chunk/entity quality guard를 유지했습니다.

| 영역 | 적용 방법 | Before | After | 결과 |
| --- | --- | ---: | ---: | ---: |
| 195-page PDF ingestion end-to-end | OpenAI metadata generation을 embedding/indexing과 병렬화하고, native-text PDF는 `pypdf` pre-classification을 생략 | 36.16s | 16.57s | 약 54% 개선 |
| Hybrid retrieval candidate gathering | 사용하지 않는 embedding/full-document column을 defer하고 lightweight BM25 projection + top-k hydration 적용 | 31.42s | 1.84s | 94.1% 개선 |
| BM25 corpus/rank/hydration | Full ORM row loading 대신 `chunk_id`/text 중심 corpus를 만들고 BM25 top-k row만 hydrate | 14.34s | 0.14s | 99.0% 개선 |

Ingestion은 OpenAI metadata generation 병렬화와 native-text PDF lazy classification으로 개선했습니다. Retrieval은 full-model corpus query를 가벼운 projection + top-k hydration으로 바꾸고 중복 SQL/embedding work를 줄였습니다. 상세 측정 조건과 남은 병목은 [performance logs](./docs/performance/README.md)에 기록합니다.
...
2. Assistant graph의 source-selection gate가 private retrieval이 필요한지 결정합니다.
3. RAG Agent가 ContextForge를 호출하고, service layer가 authorized source만 후보로 만듭니다.
...
Agent label은 실제 구현 경계를 설명합니다. 현재 production surface는 하나의 general assistant controller와 그 안에서 호출되는 RAG Agent retrieval boundary이며, 여러 specialized agent가 독립적으로 실행된다고 과장하지 않습니다.
...
- 개인, 그룹, root/system-managed knowledge base와 document-level authorization
...
- 저장소 공개 전에는 current tree 검사와 별개로 Git 전체 history를 secret scanner로 확인하고, 발견된 credential은 삭제 여부와 관계없이 폐기·재발급해야 합니다.
...
- System knowledge 관리 권한은 `root`/`system` user type으로 제한하며 public role-mutation API를 제공하지 않습니다.
...
- [General assistant graph](./my_agents/agents/general_assistant/README.md)
- [RAG Agent와 ContextForge 경계](./my_agents/agents/rag_agent/README.md)
