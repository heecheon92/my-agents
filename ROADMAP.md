...
- [~] **Read-only full-document pull tool for comprehensive document tasks.**
  - **Implemented vertical slice:** behind `MY_AGENTS_FULL_DOCUMENT_RETRIEVAL_ENABLED=false`, deterministic Korean/English intent detection activates only for explicit whole-document tasks. The graph resolves one authorized personal/group document, reuses the typed document-selection interrupt when needed, prepares coverage metadata, recalls memory, and answers through `respond_full_document`. Ordinary fact lookup, ordinary summaries, and section-specific questions stay on focused chunk retrieval.
