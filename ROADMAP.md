...
This roadmap tracks the backend functionality needed for the product chat service, using the current codebase and preserved planning decisions.
...
flowchart LR
    ...
    R1 --> R1Done["Baseline implemented; extensions remain"]
```
...
- [x] PostgreSQL-owned PostgresStore semantic memory projection with per-user opt-in, Product DB governance validation, and reconciliation; the separate `memory_graph` extraction/suggest-confirm workflow remains planned.
- [x] PostgreSQL-owned run-scoped PostgresSaver for bounded document-selection HITL/resume state; Product DB remains conversation history and audit truth.
...
- [x] System knowledge remains ambient internal context rather than a controllable interaction axis; document-selection options and resume validation exclude system KB documents.
- [x] Bounded RAG tool selection is exposed through the graph; broader tool workflows remain future work.
...
- [x] **Shipped (status mirror): Dedicated RAG Agent baseline.** `rag_agent` is the public runtime/tool boundary called by the general assistant; ContextForge is its internal permission-first focused-retrieval engine. Bounded Luna tool choice selects focused or comprehensive retrieval. See [completion record](./docs/completed/dedicated-rag-agent.md) and [canonical status](./docs/implementation-tracking.md#shipped-and-completed-index).
- [ ] Iterative evidence-driven retrieval orchestration, production hybrid tuning, and richer layout-aware ingestion remain extensions, not part of the completed boundary milestone. See [layout-aware ingestion](./docs/idea/layout-aware-ingestion-rag-agent.md).
...
- [x] Basic citation response metadata includes document/KB names, snippet, source filename/page,
  and supported `source_location_json`. Richer preview, consistent format-specific provenance,
  and confidence/source-audit UX remain follow-up work, not missing basic response fields.
...
- [x] **Shipped (status mirror):** Temporary conversation files are included in the released frontend/backend code; capability enablement and production workflow smoke remain separate. See [canonical status](./docs/implementation-tracking.md#shipped-and-completed-index) and [completion record](./docs/completed/document-workspace.md).
...
4. **Hosted ingestion performance pass**
   ...
   - Deploy `MY_AGENTS_INGESTION_EXECUTION_MODE=external_worker` with a separate `python -m my_agents.ingestion_worker` process before trying large files in production.
...
8. **Real retrieval upgrade / retrieval-agent track**
   ...
   - Extend the existing RAG Agent/ContextForge boundary instead of introducing it again. Query expansion, HyDE, layout-aware section/table tools, and deeper retrieval eval reporting remain candidates beyond the implemented hybrid retrieval/reranking baseline.
...
9. **Production ingestion upgrade**
   - Evolve the existing file upload/parser pipeline toward the layout-aware artifact plan in [`docs/idea/layout-aware-ingestion-rag-agent.md`](./docs/idea/layout-aware-ingestion-rag-agent.md).
