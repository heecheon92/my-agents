...
Classification remains deterministic; only the final reply text is generated by the selected OpenAI GPT model. Switch to deterministic mode when you need credential-free execution. Route labels in v0 are classifications for future categories only. Current product RAG, permission, and event features are service-layer behavior around the LangGraph run, not proof that separate specialized agents executed.
