...
Group/team은 **초대를 수락한 멤버만 접근할 수 있는 공유 지식 경계**입니다. 제품 API는 알려진
`user_id`로 active membership을 직접 만드는 흐름을 노출하지 않습니다. 관리자는 이메일로 초대하고,
수신자가 초대받은 이메일 계정으로 인증된 상태에서 수락해야 멤버가 됩니다.

## 제품 경계

- **초대 수락이 membership 경계입니다.** Pending invitation은 membership row가 아니며 문서/RAG 권한을 주지 않습니다.
- **사용자 탐색을 제공하지 않습니다.** 초대 생성/재전송/수락 응답은 이메일이 이미 가입된 계정인지 드러내지 않습니다.
- **공유되는 것은 group KB와 publish workflow입니다.** Conversation transcript, chat run, opt-in long-term memory는 group/team에 공유되지 않고 사용자별 비공개 범위로 남습니다.
- **Backend 용어는 `group`입니다.** 제품 화면은 “team”이라고 부를 수 있지만 v1은 organization/workspace identity management를 약속하지 않습니다.

## API contract 방향

권장 invitation surface는 다음과 같습니다.
