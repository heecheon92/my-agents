...
| `POST` | `/group-invitations/signup` | unauthenticated invited recipient | create a verified account from the token-proved email and accept membership | request includes token, nickname, password; no email field; existing accounts must sign in |
...
- no-account recipients can create a verified invited account with token, nickname, and password only;
...
- accepted member responses include display-only `nickname` while preserving duplicate nicknames and user-id role updates;
- nickname cannot be used as a login identifier; email remains the sign-in account;
...
- 2026-06-14: Added invitation-token signup for no-account recipients while preserving email login and display-only nickname semantics.
