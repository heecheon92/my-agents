...
| Document ingestion supports at least one realistic uploaded file type | 2, 6 | Upload API, parser boundary, storage metadata, parser failure tests, async progress tests, external-worker mode | Upload/ingestion UI smoke or documented gap | `05-knowledge-ingestion-extraction.md` plus frontend runbook | backend primary; frontend verifies | backend implemented for text-based PDF, Markdown, plain text, `.xlsx`, `.pptx`, and DOCX-only `.docx`; scanned/OCR-heavy files, legacy `.doc`, HTML, and CSV/JSON structural parsing remain deferred |
...
| `POST` | `/knowledge-bases/{knowledge_base_id}/documents/upload` | multipart form: `title`, `file` | `201 DocumentResponse` | KB-scoped file upload for PDF/Markdown/plain text/`.xlsx`/`.pptx`/`.docx` |
...
| Parser/file coverage beyond current local parsers | Backend supports text-based PDF, Markdown, plain-text, `.xlsx`, `.pptx`, and DOCX-only `.docx` uploads with metadata; PDFs keep page provenance and Office uploads keep Markdown artifacts when available. Scanned/encrypted/image-only PDFs, legacy `.doc`, HTML, and CSV/JSON structural parsing remain deferred. | Later parser expansion |
