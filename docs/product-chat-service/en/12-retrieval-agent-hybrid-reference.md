...
The current backend can grow toward this interface incrementally. The authorization filter remains non-negotiable: every candidate generation path must start from chunks the principal is allowed to read.
...
Current Slice A should remain a foundation, not the final architecture:
