...
| Permission-aware retrieval uses a real retrieval path, not only deterministic fixtures | 4, 6 | Embedding provider boundary, JSON cosine ranking tests, future pgvector migration/search, permission-first retrieval tests | Cited answer flow with unauthorized-doc negative proof | `06-permission-aware-rag.md` plus future retrieval doc update | backend | partially implemented; Slice A JSON embeddings exist, pgvector pending |
...
| Real embedding/vector retrieval | Slice A adds OpenAI-capable JSON-backed embeddings and permission-first cosine ranking; pgvector acceleration and cross-encoder reranking remain pending. | Phase 4 |
