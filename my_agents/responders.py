...
def _build_input_messages(
    *,
    ...
    recent_history = [_coerce_chat_message(item) for item in history[-6:]]
    messages: list[BaseMessage] = [SystemMessage(content=_SYSTEM_PROMPT)]
    messages.extend(_to_langchain_message(item) for item in recent_history)
    messages.append(
        HumanMessage(
            content=(
...
def _coerce_chat_message(item: ChatMessage) -> ChatMessage:
