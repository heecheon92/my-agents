...
- When full-document retrieval is enabled, deterministic Korean/English intent detection routes only explicit whole-document tasks through `resolve_full_document_target -> prepare_full_document_read -> retrieve_memory -> respond_full_document`.
...
The opt-in comprehensive branch is narrower than ordinary retrieval. `classify_request` requires both a whole-document phrase and a task verb, so “What does full document retrieval mean?” and a normal “Summarize this document” do not activate it. The branch resolves exactly one currently authorized user-controllable document by resumed selection, sole eligible target, or unique title/filename match. Ambiguity reuses the existing typed document-selection interrupt when checkpointer support is available. Ambient system documents are excluded from target resolution, options, resume values, and range reads.
