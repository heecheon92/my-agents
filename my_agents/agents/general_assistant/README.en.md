...
| authorized documents / system project knowledge | service-layer `retrieved_context` | Already permission-filtered before entering the graph. System/project knowledge may be ambient retrieval context for all authenticated users, but it is not user memory |
