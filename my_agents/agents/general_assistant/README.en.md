...
| authorized documents | graph-owned `retrieve_rag_context` call into the RAG Agent runtime | Prompt-safe context that already passed permission filtering; ambient system/project entries contain snippet text only, while personal/group entries may keep ordinary provenance. |
