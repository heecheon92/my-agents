...
def test_alembic_offline_sql_generation_covers_initial_schema(monkeypatch) -> None:
    ...
    assert "20260615_0028" in sql
    assert "20260624_0029" in sql
    ...
    assert "source_document_title_snapshot" in sql
    assert "source_knowledge_base_name_snapshot" in sql
    assert "published_knowledge_base_name_snapshot" in sql
    assert "CREATE TABLE conversation_attachments" in sql
    assert "CREATE TABLE document_workspaces" in sql
    assert "CREATE TABLE conversation_artifacts" in sql
    assert "CREATE TABLE usage_events" in sql
    assert "reasoning_mode" in sql
...
def test_legacy_documents_without_knowledge_base_upgrade_to_head(
    ...
    assert alembic_version == "20260809_0031"
